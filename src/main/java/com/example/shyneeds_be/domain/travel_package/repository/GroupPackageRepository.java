...
public interface GroupPackageRepository extends JpaRepository<TravelPackage, Long> {
    ...
    @Query(value = "SELECT " +
            "TP.id AS travelId, TP.title AS travelTitle, TP.main_image AS mainImage, TP.search_keyword AS searchKeyword, " +
    ...
    @Query(value = "SELECT TP.id AS travelId, TP.title AS travelTitle, TP.main_image AS mainImage, TP.search_keyword AS searchKeyword, " +
            "SCA.id AS subCategoryId, SCA.title AS subCategoryTitle " +
            "FROM shyneeds.travel_package AS TP " +
            "INNER JOIN shyneeds.sub_category AS SCA " +
